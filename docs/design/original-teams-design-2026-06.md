# Agent 桌面版 — Teams 系統最初設計草稿（歷史文件）

> ⚠️ **本文件已過時，僅作歷史記錄保留**。整理自 2026-06-26 的設計討論，
> 記錄 Teams／HR Agent 自動組隊功能最初的規劃思路。文件內「尚未開始」
> 的狀態描述已不準確——下述 Phase 1-4 描述的功能目前都已實作完成，
> 其中 Phase 4「HR Agent 自動組隊」後續又被重新設計並移除（改用單一
> Agent + Skills 就能處理同樣的情境，不需要獨立的自動組隊層），最終
> 演變成現在的「Project（深度組隊）」與「團隊對話」功能，跟本文件
> 描述的設計已有落差。
>
> 想了解目前實際功能，請見根目錄 [README.md](../../README.md)；
> 想了解目前進行中的工作路線圖，請見
> [ENHANCEMENT-ROADMAP.md](../ENHANCEMENT-ROADMAP.md)。

---

> 原始文件開頭（整理自 2026-06-26 設計討論，內容保留供歷史參考）：
>
> 未完成的設計決策以 ❓ 標記。
>
